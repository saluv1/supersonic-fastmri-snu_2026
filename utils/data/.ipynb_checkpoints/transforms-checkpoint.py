...
class DataTransform:
    ...
    3. Preserve both the union bbox mask and individual transformed boxes.
    4. Optionally augment the k-space sampling mask.
    5. Apply the sampling mask and format tensors for PromptMR.

    Training/validation return eight items:

        mask, kspace, target, maximum, fname, slice, bbox_mask, boxes

    ``bbox_mask`` is retained for backward compatibility and smoke tests.
    ``boxes`` keeps each transformed lesion separate so the training loss and
    validation metric can average boxes in the same way as the leaderboard.

    Forward inference keeps the original six-item return format because
    annotation supervision is not used during inference.
    """
    ...
    def __call__(
        ...
        if not self.isforward:
            # Keep the old union mask for compatibility with existing smoke
            # tests. The new per-box loss will use ``boxes`` instead.
